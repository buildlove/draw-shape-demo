...
PolygonDrawingTool.prototype.doStop = function() {
  ...
  this._polygons = this._polygons.concat([this._polygon])
  this._polygon = []
  console.log(this._polygons)
  window.test = this._polygons
...
PolygonDrawingTool.prototype.addPoint = function(p) {
  this._polygon = this._polygon.concat([JSON.parse(JSON.stringify({
    F: p.F, G: p.G, v: true,
    x:p.F, y:p.G
  }))])
  ...
  var pt = new go.Point(p.x - viewpt.x, p.y - viewpt.y)
  var q = this.modifyPointForGrid(pt);
